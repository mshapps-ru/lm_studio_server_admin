...
The controller also handles timeouts (5 min) and graceful error responses when the LM Studio service is unreachable. When verbose logging is enabled via `verboseLogging` in `config.json`, it logs detailed request and response data for debugging purposes.
